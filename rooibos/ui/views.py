...
def remove_tag(request, type, id):
    ...
    if request.method == 'POST':
        if request.user.is_staff:
            ownedwrapper = OwnedWrapper.objects.filter(object_id=id)
        else:
            ownedwrapper = OwnedWrapper.objects.get_for_object(user=request.user, type=type, object_id=id)
        for wrap in ownedwrapper:
            tag_names = Tag.objects.get_for_object(wrap).exclude(name=tag).values_list('name')
            tag_str =' '.join(map(lambda s: '"%s"' % s, tag_names))
            Tag.objects.update_tags(wrap, tag_str)
